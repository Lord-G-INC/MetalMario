...
void MetalMario::control() {
    ...
    if (ma->mPlayerMode == 14) {
        ...
        if (mElapseTimer) 
            if (!--mElapseTimer) 
                this->invalidateMetal();

        if (MR::isPlayerInAreaObj("WaterArea")) {
            OSReport("I wet myself\n");
            ma->mVelocity *= 0.6; // obj_arg !!!
...
void MetalMario::attackSensor (HitSensor *pSender, HitSensor *pReceiver) {
    if (mIsPlayerElectric && MR::isEqualSubString(pReceiver->mActor->mName, "コイン")) { // Coin
...
bool electrifyPlayer (HitSensor *pReceiver, u32 msg, HitSensor *pSender) {
    if (MR::isSensorPlayer(pReceiver)) {
        ...
        if (ma->mPlayerMode == 14) {
            ...
            for (int i = 0; i < 6; i++) 
                gMetalPlayer->mParts[i]->mScale = TVec3f(0.5);
            return false;
        } else 
...
void validateMetalPlayer () {
    ...
    // Figure out how to change tracks (IDA!!!)
    removeAllClingingKarikari__2MRFv();
    gMetalPlayer->mElapseTimer = 2400;
}

// For some reason Rock is now 0xD instead of 0xC? May need to investigate...
...
void MetalPlayerElectricPad::attackSensor (HitSensor *pReceiver, HitSensor *pSender) {
    if (MR::isEqualString(pSender->mActor->mName, "マリオアクター") && gMetalPlayer->mIsPlayerElectric) {
